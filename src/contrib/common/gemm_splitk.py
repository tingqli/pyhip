    ...
    def mfma_gen(pp_reg_id, k=None):
        if weight_dtype == torch.float4_e2m1fn_x2:
            ...
            # cdna4 path:
                ...
                for i in range(2):
                    for n in range(B_horz):
                        # B_reg = J.gpr(2, B_horz, 2, 2, "vbf16x2")
                        J.v_cvt_scalef32_pk_bf16_fp8(v_w_bf16[n, 0], B_reg[pp_reg_id, n, i, 0], 1.0)
                        J.v_cvt_scalef32_pk_bf16_fp8(v_w_bf16[n, 1], B_reg[pp_reg_id, n, i, 0], 1.0, mod='op_sel:[1,0,0]')
                        J.v_cvt_scalef32_pk_bf16_fp8(v_w_bf16[n, 2], B_reg[pp_reg_id, n, i, 1], 1.0)
                        J.v_cvt_scalef32_pk_bf16_fp8(v_w_bf16[n, 3], B_reg[pp_reg_id, n, i, 1], 1.0, mod='op_sel:[1,0,0]')
                    for n in range(B_horz):
                        # 2, A_vert, A_rep=2, 2, 2
                        for m in range(A_vert):
