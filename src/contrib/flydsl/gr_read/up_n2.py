...
"""GR read Up N2：完整K320、两个H16子阶段，FP32 logits与整数helper Y打包。
...
@cache
def make_up_n2(rows, padded_rows):
    ...
    def gr_read_up_n2(X: fx.Tensor, W: fx.Tensor, P: fx.Tensor, Y: fx.Tensor):
    ...
    def launch_up(X: fx.Tensor, W: fx.Tensor, P: fx.Tensor, Y: fx.Tensor, stream: fx.Stream):
        gr_read_up_n2(X, W, P, Y).launch(grid=((rows + BM - 1) // BM, N_SPLITS, 1), block=(512, 1, 1), stream=stream)
