...
__all__ = ["moe_gemm_final_reduce_bf16", "moe_gemm_8wave_g1u1", "moe_gemm_8wave_down"]
    ...
    with J.While(tok0[0] < tok1[0]):
        ...
        for part_id in range(part_cnt):
            ...
            if part_id == (part_cnt - 2):
                voffset_prefetch[0] += (
                    TOPK * OC - OC
                ) * J.sizeof_bf16  # go to next token
            index = index ^ 1
...
def moe_gemm_8wave_g1u1(
    ...
    lds_base = J.alloc_lds(
        HALF_BLOCK_SIZE_ROW * BLOCK_K * 4 + HALF_BLOCK_SIZE_COL * BLOCK_K * 4
    )
    ldsA = {}
    ldsB = {}
    lds = lds_base
    ...
    ldsA[0, 0] = lds
    lds += HALF_BLOCK_SIZE_ROW * BLOCK_K
    ldsA[0, 1] = lds
    lds += HALF_BLOCK_SIZE_ROW * BLOCK_K
    ldsA[1, 0] = lds
    lds += HALF_BLOCK_SIZE_ROW * BLOCK_K
    ...
    ldsB[0, 0] = lds
    lds += HALF_BLOCK_SIZE_COL * BLOCK_K
    ldsB[0, 1] = lds
    lds += HALF_BLOCK_SIZE_COL * BLOCK_K
    ldsB[1, 0] = lds
    lds += HALF_BLOCK_SIZE_COL * BLOCK_K
    ...
    vm_load_a, vm_load_cnt_a, vm_offset_inc_a, ds_read_a = get_mfma_loader_sorted_tok(
        J,
        num_warps,
        BLOCK_SIZE_ROW,
        BLOCK_K,
    ...
    vm_load_b, vm_load_cnt_b, vm_offset_inc_b, ds_read_b = get_mfma_loader(
        J,
        bpreshuffle,
        num_warps,
        HALF_BLOCK_SIZE_COL,
    ...
    if use_f32_blockscales_128:
        ...
        if gate_up:
            buff_sa = J.Buffer(
                pScaleA, num_tokens[0] * (J.div(K, scale_BK) * J.sizeof_u32)
            )
            voffset_scaleA = J.gpr((vrows[0] & 0xFFFFFF) * J.sizeof_u32)
        else:
            buff_sa = J.Buffer(
                pScaleA, num_tokens[0] * (TOPK * J.div(K, scale_BK) * J.sizeof_u32)
            )
            voffset_scaleA = J.gpr(
                (vrows[0] & 0xFFFFFF) * (TOPK * J.sizeof_u32)
        ...
        def vm_load_scaleA(lds, bk):
            ...
            if gate_up:
                voff = J.gpr(
                    "vu32",
                    voffset_scaleA[0]
                    + J.gpr("su32", num_tokens[0] * (bk * J.sizeof_u32)),
                )
            else:
                voff = J.gpr(
                    "vu32",
                    voffset_scaleA[0]
                    + J.gpr("su32", num_tokens[0] * (TOPK * bk * J.sizeof_u32)),
                )
        ...
        pScaleB[:] += (
            expert_id * J.div(OC, scale_BN) * J.div(K, scale_BK) * J.sizeof_f32
        )
        lds_scaleB = J.alloc_lds(
            J.div(K, scale_BK) * J.div(wg_N, scale_BN) * J.sizeof_f32
        )
        if gate_up:
            # first half from gate, second half from up
            assert wg_N >= 2 * scale_BN
            pScaleB[:] += blk_n * (
                J.div(wg_N, 2, scale_BN) * J.div(K, scale_BK) * J.sizeof_f32
            )
            J.wg_load_lds(
                lds_scaleB,
                pScaleB,
            ...
            pScaleB[:] += blk_n * (
                J.div(wg_N, scale_BN) * J.div(K, scale_BK) * J.sizeof_f32
            )
            J.wg_load_lds(
                lds_scaleB,
                pScaleB,
        ...
        vaddr_scaleA = J.gpr(
            "vu32",
            (J.lane_id[0] % 16) * J.sizeof_f32 + warp_m * (16 * nrM * J.sizeof_f32),
        )
    ...
    if use_f32_blockscales_128:
        ...
        def mfma(c_index):
            ...
            for m in range(nrM):
                for n in range(nrN):
                    if n == 0:
                        mfma_fifo_scale[c_index % 2, m] = (
                            mfma_scaleA[m] * mfma_scaleB[b_index]
                        )
                    J.v_fmac_f32(
                        mfma_C[mfma_fifo_c_index, m, n, 0],
                        mfma_fifo[fifo_read_id, 0],
                        mfma_fifo_scale[mfma_fifo_c_index % 2, m],
                    )
                    J.v_fmac_f32(
                        mfma_C[mfma_fifo_c_index, m, n, 1],
                        mfma_fifo[fifo_read_id, 1],
                        mfma_fifo_scale[mfma_fifo_c_index % 2, m],
                    )
                    J.v_fmac_f32(
                        mfma_C[mfma_fifo_c_index, m, n, 2],
                        mfma_fifo[fifo_read_id, 2],
                        mfma_fifo_scale[mfma_fifo_c_index % 2, m],
                    )
                    J.v_fmac_f32(
                        mfma_C[mfma_fifo_c_index, m, n, 3],
                        mfma_fifo[fifo_read_id, 3],
                        mfma_fifo_scale[mfma_fifo_c_index % 2, m],
                    )
                    fifo_read_id += 1

                    J.v_mfma_f32_16x16x128_f8f6f4(
                        mfma_fifo[fifo_write_id % MFMA_FIFO_CNT],
                        mfma_B[b_index, n],
                        mfma_A[m],
                        0,
                    )
    ...
    if 1:
        ...
        toc = 1
        if use_f32_blockscales_128:
            vm_load_scaleA(lds_scaleA[tic], 0)
        ...
        J.s_waitcnt(mod=f"vmcnt({vm_load_cnt_a + vm_load_cnt_b})")
        J.s_barrier()
        ...
        J.s_waitcnt(
            mod=f"vmcnt({vm_load_cnt_a + vm_load_cnt_b*2 + vm_load_cnt_scaleA})"
        )
        J.s_barrier()
        ...
        if 1:
            ...
            assert (
                not use_f32_blockscales_128
            ), "there is an unknown accuracy issue for f32 blockscale-128 case"
            assert (loop_cnt % 2) == 0
            k = J.gpr("su32", 0)
    ...
    if gate_up:
        ...
        if nrN == 1:
            ...
            vaddr0 = J.gpr(
                "vu32",
                col * J.sizeof_DW2
                + warp_n * 16 * J.sizeof_bf16
                + blk_n * (J.div(wg_N, 2) * J.sizeof(C_dtype)),
            )
        else:
        ...
        for cm in range(2):
            ...
            for m in range(nrM):
                ...
                with J.ExecMask(vrows_topk < TOPK):
                    ...
                    J.v_lshl_add_u64(
                        vaddr,
                        J.gpr(2, "vu32", vaddr0 + vrows_topk * (stride_n), 0),
                        0,
                        vaddr,
                    )
                    J.v_mad_u64_u32(
                        ...
                        "vcc",
                        (vrows[cm, m] & 0xFFFFFF),
                        J.gpr("vu32", TOPK * stride_n),
                        vaddr,
                    )
                    for n in range(nrN):
                        mfma_C[igate, m, n, 0] = (
                            J.silu(mfma_C[igate, m, n, 0]) * mfma_C[iup, m, n, 0]
                        )
                        mfma_C[igate, m, n, 1] = (
                            J.silu(mfma_C[igate, m, n, 1]) * mfma_C[iup, m, n, 1]
                        )
                        mfma_C[igate, m, n, 2] = (
                            J.silu(mfma_C[igate, m, n, 2]) * mfma_C[iup, m, n, 2]
                        )
                        mfma_C[igate, m, n, 3] = (
                            J.silu(mfma_C[igate, m, n, 3]) * mfma_C[iup, m, n, 3]
                        )
                    assert nrN in [1, 2], f"{nrN=}"
                    if nrN == 1:
                        J.uni_cvt_pk_bf16_f32(
                            vbf16[0], mfma_C[igate, m, 0, 0], mfma_C[igate, m, 0, 1]
                        )
                        J.uni_cvt_pk_bf16_f32(
                            vbf16[1], mfma_C[igate, m, 0, 2], mfma_C[igate, m, 0, 3]
                        )
                        # buff_c.store_dwordx2(vbf16[0:1], vaddr, 0, offset12 = 0*16*J.sizeof(C_dtype))
                        J.global_store_dwordx2(
                            vaddr,
                            vbf16[0:1],
                            "off",
                            mod=f"offset:{0*16*J.sizeof(C_dtype)}",
                        )
                    else:
                        J.uni_cvt_pk_bf16_f32(
                            vbf16[0], mfma_C[igate, m, 0, 0], mfma_C[igate, m, 0, 1]
                        ...
                        J.uni_cvt_pk_bf16_f32(
                            vbf16[2], mfma_C[igate, m, 1, 0], mfma_C[igate, m, 1, 1]
                        )
                        J.uni_cvt_pk_bf16_f32(
                            vbf16[3], mfma_C[igate, m, 1, 2], mfma_C[igate, m, 1, 3]
                        )
                        ...
                        J.global_store_dwordx4(
                            vaddr, vbf16, "off", mod=f"offset:{0*16*J.sizeof(C_dtype)}"
                        )
    else:
        # scatter output to : [num_tokens, topk, dims]
        vrows = J.gpr(2, nrM, "vu32")
        ...
        for cindex in range(4):
            ...
            for m in range(nrM):
                ...
                with J.ExecMask(vrows_topk < TOPK):
                    ...
                    J.v_lshl_add_u64(
                        vaddr,
                        J.gpr(2, "vu32", vaddr0 + vrows_topk * (stride_c), 0),
                        0,
                        vaddr,
                    )
                    J.v_mad_u64_u32(
                    ...
                    if nrN == 1:
                        ...
                        J.uni_cvt_pk_bf16_f32(
                            vbf16[0], mfma_C[cindex, m, n, 0], mfma_C[cindex, m, n, 1]
                        )
                        J.uni_cvt_pk_bf16_f32(
                            vbf16[1], mfma_C[cindex, m, n, 2], mfma_C[cindex, m, n, 3]
                        )
                        ...
                        J.global_store_dwordx2(
                            ...
                            vbf16[0:1],
                            "off",
                            mod=f"offset:{n*16*J.sizeof(C_dtype) + cn*HALF_BLOCK_SIZE_COL*J.sizeof_bf16}",
                        )
                    else:
                        for n in range(0, nrN, 2):
                            ...
                            J.v_mul_f32(
                                mfma_C[cindex, m, n + 1, 0],
                                mfma_C[cindex, m, n + 1, 0],
                                vweights[cm, m],
                            )
                            J.v_mul_f32(
                                mfma_C[cindex, m, n + 1, 1],
                                mfma_C[cindex, m, n + 1, 1],
                                vweights[cm, m],
                            )
                            J.v_mul_f32(
                                mfma_C[cindex, m, n + 1, 2],
                                mfma_C[cindex, m, n + 1, 2],
                                vweights[cm, m],
                            )
                            J.v_mul_f32(
                                mfma_C[cindex, m, n + 1, 3],
                                mfma_C[cindex, m, n + 1, 3],
                                vweights[cm, m],
                            )
                            ...
                            J.global_store_dwordx4(
                                ...
                                vbf16,
                                "off",
                                mod=f"offset:{0*16*J.sizeof(C_dtype) + cn*HALF_BLOCK_SIZE_COL*J.sizeof_bf16}",
                            )
...
def moe_gemm_8wave_down(
    ...
    with J.While():
        ...
        with J.ExecMask(J.threadIdx.x[0] == 0):
            J.global_atomic_add(
                vnext_blk_id, vaddr_zeros, vone, blk_atomic_int, mod="sc0"
            )
            J.s_waitcnt(mod=f"vmcnt(0)")
        ...
        nrK = J.div_up(
            IC * J.sizeof(AB_dtype), 64
        )  # always use 64-bytes in K dims (due to dwordx4/b128)
        mfma_A = J.gpr(
            nrM, nrK, 4, "vbf16x2"
        )  # 4 b32 regs in MFMA-16 layout : 16x16xfp32/16x32xbf16/16x64xfp8
        ...
        if AB_dtype == "fp8":
            assert (
                nrK >= 2 and (nrK % 2) == 0
            ), f"v_mfma_f32_16x16x128_f8f6f4 needs K=128, but {nrK=}"
        ...
        for m in range(nrM):
            ...
            with J.ExecMask(vrows[m] < num_token_topks):
                ...
                for k in range(nrK):
                    J.global_load_dwordx4(
                        mfma_A[m, k], vaddr, "off", mod=f"offset:{k*64}"
                    )
        ...
        num_vm_loads = J.div(
            num_16x64b_wg_N * num_16x64b_K, num_warps
        )  # 4 @ num_warps=4
        ...
        if AB_dtype == "bf16":
            ...
            for bm in range(nrM):
                ...
                _voff[0] = J.gpr(
                    "vu32",
                    J.warp_id[0] * (warp_M * J.sizeof_u32)
                    + (bm * 16 * J.sizeof_u32)
            ...
            J.wg_load_lds(
                lds_scaleB, pScaleB, sizeof_scaleB, num_warps, wait_barrier=False
            )
                ...
                for bn in range(nrsN):
                    for bk in range(num_scales_K):
                        vaddr = J.gpr(
                            "vu32",
                            lds_scaleB
                            + (bn + bn_wgN) * J.div(IC, scale_BK) * J.sizeof_f32
                            + bk * J.sizeof_f32,
                        )
            ...
            def mfma():
                ...
                for k in range(0, nrK, 2):
                    for m in range(nrM):
                        for n in range(nrN):
                            ...
                            mfma_scaleAB = J.gpr(
                                "vf32",
                                mfma_scaleA[m, k * 64 // scale_BK]
                                * mfma_scaleB[n * 16 // scale_BN, k * 64 // scale_BK],
                            )
                            J.v_mfma_f32_16x16x128_f8f6f4(
                                temp, mfma_B[n, k : k + 1], mfma_A[m, k : k + 1], 0
                            )
                            # J.s_nop(4)
                ...
                while len(dequant_queue):
                    tc, ts, (tm, tn) = dequant_queue.pop(0)
                    # IC=128 has only one K block; the tail may be C's first write.
                    if mfma_C_initialized[tm, tn] == 0:
                        mfma_C_initialized[tm, tn] = 1
                        J.v_mul_f32(mfma_C[tm, tn, 0], tc[0], ts)
                        J.v_mul_f32(mfma_C[tm, tn, 1], tc[1], ts)
                ...
                for m in range(nrM):
                    for n in range(0, nrN, 2):
                        ...
                        for i in range(4):
                            J.v_mul_f32(
                                mfma_C[m, n + 1, i], mfma_C[m, n + 1, i], vweights[m]
                            )

                        J.uni_cvt_pk_bf16_f32(
                            mfma_C_bf16[m, n, 0], mfma_C[m, n, 0], mfma_C[m, n, 1]
                        )
                        J.uni_cvt_pk_bf16_f32(
                            mfma_C_bf16[m, n, 1], mfma_C[m, n, 2], mfma_C[m, n, 3]
                        )
                        J.uni_cvt_pk_bf16_f32(
                            mfma_C_bf16[m, n, 2],
                            mfma_C[m, n + 1, 0],
                        ...
                        J.v_permlane16_swap_b32(
                            mfma_C_bf16[m, n, 0], mfma_C_bf16[m, n, 2]
                        )
                        J.v_permlane16_swap_b32(
                            mfma_C_bf16[m, n, 1], mfma_C_bf16[m, n, 3]
                        )
        ...
        for m in range(nrM):
            if is_output_over_4GB:
                J.v_mad_u64_u32(
                    vaddr_rows[m], "vcc", vrows[m], J.gpr("su32", stride_c), 0
                )
                ...
                J.v_lshl_add_u64(
                    ...
                    J.gpr(
                        ...
                        "vu32",
                        swap_12_col * J.sizeof_DW4
                        + J.gpr("su32", blk_oc * OC * J.sizeof(C_dtype)),
                        0,
                    ),
                    0,
                    vaddr_rows[m],
        ...
        def storeC(block_n):
            ...
            for m in range(nrM):
                with (
                    J.ExecMask(vrows[m] < num_token_topks, early_skip=False)
                    if is_output_over_4GB
                    ...
                    for n in range(0, nrN, 2):
                        if is_output_over_4GB:
                            ...
                            J.v_lshl_add_u64(
                                vaddr_64bits, vaddr_64bits, 0, vaddr_rows[m]
                            )
                            J.global_store_dwordx4(
                                vaddr_64bits,
                                mfma_C_bf16[m, n],
                                "off",
                                mod=f"offset:{n*16*J.sizeof(C_dtype)}",
                            )
                        else:
                            buff_c.store_dwordx4(
                                ...
                                vaddr_rows[m, 0],
                                soffset,
                                offset12=n * 16 * J.sizeof(C_dtype),
                                ext_mod="",
                            )
                        yield 64
