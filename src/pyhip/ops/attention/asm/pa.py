...
def pa_jit(J:pyhip.JIT,
    ...
    def reduce(J:pyhip.JIT,
        ...
        with J.ExecMask(lane_div_16[0] == 0):
            ...
            addr = J.Addr2D(lds_base, warp_id, lane_mod_16 * 4, 16 * 4)
        ...
        for i in range(gqa4):
            ...
            addr = J.Addr2D(lds_base, 0, m_token_id * 4, 16 * 4)
        ...
        addr = J.Addr2D(lds_base, warp_id, S * 2 * lane_mod_16, 16 * S * 2)
        for k in range(S // 64):
        ...
        for i in range(gqa4):
            m_token_id = gqa4 * warp_id + i
            with J.ExecMask(m_token_id < GQA):
                addr = J.Addr2D(lds_base, m_token_id, ((m_token_id ^ (lane_id >> 1)) * 2 + (lane_id & 1)) * 4, S * 2)
                tmp = J.gpr(4, 'vf32')
