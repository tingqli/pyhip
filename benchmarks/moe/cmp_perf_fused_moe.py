...
def get_fused_moe_time(model_dim, inter_dim, num_tokens, experts, topk, quant_mode, ep_size, use_jit):
    script = Path(__file__).with_name("test_fused_moe.py")
    cmd = f"{shlex.quote(sys.executable)} {shlex.quote(str(script))} -dim {model_dim},{inter_dim} -t {num_tokens} -a silu -s f -e {experts} -k {topk} -p t -q {quant_mode} -ep {ep_size}"
    if use_jit:
        cmd += " -j"
