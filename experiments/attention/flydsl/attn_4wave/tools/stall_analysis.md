...
> [1K--32K性能报告](../../../../../tests/contrib/moe/MAIN_MERGE_PERFORMANCE_REPORT.md)。

> 2026-09-06七K当前PTPC baseline已重新采集并复制到仓库根，逐文件校验与入口见
> [compact验收/UI清单](../../../../../tests/contrib/moe/COMPACT_M64_REPORT.md#七k-baseline-ui)。
...
> 上述为K512原型阶段。最新按要求[所有七K默认1N循环及全链验收](../../../../../tests/contrib/moe/ALL_8X1_NLOOP_REPORT.md)已完成：
...
```bash
python experiments/attention/flydsl/attn_4wave/tools/analyze_mfma_stall.py \
...
- [静态审计](../../../../../tests/contrib/moe/results/k512_vmcnt5/isa_audit_v2.json)按寄存器定义追踪普通8x1、compact满块：
  每份N2048 ISA均128条Bload→LDS-write依赖全部被wait覆盖，58处vmcnt(5)的最后五条均为上述1＋2＋2。
- [专项脚本](../../../../../tests/contrib/moe/check_k512_vmcnt.py)未修改生产kernel，编译当前版本与仅将VM字段5改为0的保守版本。
  四份实际ISA逐指令对照证明：除了`vmcnt(5)→vmcnt(0)`外完全一致，寄存器/数学指令不变。
- [数值结果](../../../../../tests/contrib/moe/results/k512_vmcnt5/gpu_check/result.json)：K512、E8/TopK4、随机routing/随机FP8权重，
...
**实测证据：** [选择性9三版本结果](../../../../../tests/contrib/moe/results/k512_vmcnt9/gpu_check/result.json)。
...
- [CPU单测](../../../../../tests/contrib/moe/test_check_k512_vmcnt.py)6项通过，包含9条后继的正例、N边界不足9条的反例。
...
- [机器可读汇总](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/summary.json)记录各轮源码/ISA哈希、
  资源、原始样本复算、硬件状态及非目标回归；[专用汇总脚本](../../../../../tests/contrib/moe/summarize_k512_vmcnt9.py)
  不修改任何输入JSON，不将历史5版结果重标为当前9版。

#### 正确性、基线身份与资源

[生产三版本专项](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/correctness/result.json)：
...
- **120项回归通过，929.66秒**：compact down/完整链100项、普通8x1七K×两量化14项、等待账本CPU6项。
  本轮未重复声称覆盖此前全部175项；测试入口为[compact测试](../../../../../tests/contrib/moe/test_compact_m64_down.py)、
  [普通8x1测试](../../../../../tests/ops/moe/test_moe.py)、[等待账本测试](../../../../../tests/contrib/moe/test_check_k512_vmcnt.py)。
- 性能harness另增加[4项ISA分组正反例](../../../../../tests/contrib/moe/test_benchmark_k512_vmcnt9.py)，全部通过。
  首次冒烟错误地要求整份compact汇编没有9，误计了**1x4尾kernel原有的5处编译器`vmcnt(9)`**；
  已修为仅对full8x1计数/替换，tasks/tail逐指令原样比较。
  [失败冒烟](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/qwen35_smoke.json)无性能样本，
  [修复后冒烟](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/qwen35_smoke_v2.json)通过，不作正式收益依据。
...
[性能harness](../../../../../tests/contrib/moe/benchmark_k512_vmcnt9.py)每次同进程测
...
确认轮原始证据：[35B确认48轮](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/qwen35_confirm_abba48.json)、
[397B确认48轮](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/qwen397_confirm_abba48.json)。
...
首轮原始证据：[35B首轮48轮](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/qwen35_abba48.json)、
[397B首轮48轮](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/qwen397_abba48.json)。
...
- [采集器](../../../../../tests/contrib/moe/capture_k512_vmcnt_att.py)复用现有down-only工作负载：新9原生编译，
...
- 旧5首次尝试在设置硬件后的全机门禁遇到最高17% busy，未启动rocprof采样；
  [拒绝记录](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/att_old5/hardware.json)及恢复状态保留。
  CPU复算新9后重新核验门禁，使用全新retry目录成功采集，没有放宽阈值。

身份与逐文件校验见[旧5采集清单](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/att_old5_retry1/capture.json)、
[新9采集清单](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/att_new9/capture.json)。
...
- [旧5七层账本](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/att_old5_retry1/analysis.json)
  与[新9七层账本](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/att_new9/analysis.json)。
- [完整fresh对照审计](../../../../../tests/contrib/moe/results/k512_vmcnt9_production/att_comparison.json)由
  [ATT汇总脚本](../../../../../tests/contrib/moe/summarize_k512_vmcnt_att.py)复核原件/副本全部hash、真实resident重叠及四类闭合。
- [decoder映射3项CPU测试](../../../../../tests/contrib/moe/test_capture_k512_vmcnt_att.py)、等待分组4项、分析器14项，
...
[新审计](../../../../../tests/contrib/moe/check_k512_stage4.py)除B→LDS外，还沿实际指令检查**全部普通VMEM load结果的读取及寄存器重定义**。
当前模型限gfx942普通buffer/global、直线展开active主体，不声称覆盖任意FLAT/未知CFG。

#### 已完成的专项验证

[随机FP32参考及旧源码对照](../../../../../tests/contrib/moe/results/k512_stage4_vmcnt5/correctness/result.json)：
...
[8项CPU测试](../../../../../tests/contrib/moe/test_check_k512_stage4.py)包括首末N、scale提前使用/寄存器提前覆盖反例、
...
但未完成部分仍属未验证，精确状态见[回归记录](../../../../../tests/contrib/moe/results/k512_stage4_vmcnt5/regression_status.json)。
397B完整矩阵随后按用户“快速测试”要求在编译new_compact时中止、无完整性能样本；
普通旧/新ELF已经生成，直接复用做下节快速对照。不拿上一节stage4仍为1的ATT解释当前ISA。

#### 性能对照协议

[stage4专用harness](../../../../../tests/contrib/moe/benchmark_k512_stage4.py)只对比stage4=1/5，两侧stage1–3均为9。
...
结束恢复原auto/PTL Disabled/N/A/NUMA=1。两轮两buffer的[冒烟](../../../../../tests/contrib/moe/results/k512_stage4_vmcnt5/qwen35_smoke.json)
功能/ISA检查通过，但退出busy7%，仅保留功能证据，不作性能收益；正式48轮使用新文件独立门禁。
[汇总器](../../../../../tests/contrib/moe/summarize_k512_stage4.py)拒绝缺失phase/path/样本/逐轮合同或非法时延，
[完整性反例测试](../../../../../tests/contrib/moe/test_summarize_k512_stage4.py)及其它CPU审计合计31项通过（含前述8项），0.19秒。
...
[快速入口](../../../../../tests/contrib/moe/benchmark_k512_quick.py)从已经生成的binary MLIR提取原始ELF，
...
证据：[12轮快速结果](../../../../../tests/contrib/moe/results/k512_stage4_vmcnt5/quick_abba12_retry1.json)、
[40轮确认](../../../../../tests/contrib/moe/results/k512_stage4_vmcnt5/quick_confirm_abba40.json)。
...
此前已完成的[35B完整48轮](../../../../../tests/contrib/moe/results/k512_stage4_vmcnt5/qwen35_abba48.json)也保留：
...
上述原型及验收项已记入[TODO](../../../../../tests/contrib/moe/TODO.md)；后续实现与实测如下，默认仍保留全展开。
...
- [当时的主builder快照](../../../../../tests/contrib/moe/results/k512_nloop/final_n2048_u1/sources/gemm2_8x1.py)新增私有`_n_loop=0`参数。
...
- [独立N循环原型快照](../../../../../tests/contrib/moe/results/k512_nloop/final_n2048_u1/sources/gemm2_8x1_k512_nloop.py)
...
使用[小范围编译/参考脚本](../../../../../tests/contrib/moe/check_k512_nloop.py)，同一Python3.10/FlyDSL0.3.2/
...
[CPU循环测试](../../../../../tests/contrib/moe/test_k512_nloop.py)覆盖N逐tile恰好一次、末N预取边界、动态B位置与原公式一致、
正式stage4默认及不支持参数拒绝。沿用之前stage4的160次专项和78项后中断的广泛回归记录，
不把本次针对性检查伪装成重跑七K/Full全集。

#### 40轮Down-only确认（每次仅18–21秒）

[零JIT性能入口](../../../../../tests/contrib/moe/benchmark_k512_nloop.py)直接加载已有ELF。
...
四份确认：[2048/1N](../../../../../tests/contrib/moe/results/k512_nloop/n2048_u1_abba40.json)、
[2048/2N](../../../../../tests/contrib/moe/results/k512_nloop/n2048_u2_abba40.json)、
[4096/1N](../../../../../tests/contrib/moe/results/k512_nloop/n4096_u1_abba40_retry1.json)、
[4096/2N](../../../../../tests/contrib/moe/results/k512_nloop/n4096_u2_abba40.json)。
...
所有源码、边界、资源、回边工作量与配对统计由[汇总器](../../../../../tests/contrib/moe/summarize_k512_nloop.py)
重算，机器证据见[最终验收汇总](../../../../../tests/contrib/moe/results/k512_nloop/summary.json)。
...
- [随机torch-reference结果](../../../../../tests/contrib/moe/results/multik_busy80/current_k512_check.json#L1)：
  B257、E8、TOPK4、N2048，relative-L2 `0.0033062608`、finite；与原版control逐bit一致。
- [10-buffer ABBA24](../../../../../tests/contrib/moe/results/multik_busy80/current_k512_abba24.json#L1)：
...
- [七层机器账本](../../../../../tests/contrib/moe/results/multik_busy80/current_k512/analysis.json#L1)；
...
- [采集命令及硬件状态](../../../../../tests/contrib/moe/results/multik_busy80/current_k512/hardware.json#L1)。
...
在[该轮主builder快照](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_clean/ui_output_agent_53149_dispatch_22/source_2_gemm2_8x1.py#L77)中增加
`DEDICATED_K512`，只对K512/BK128启用rolling；新增独立的
[`pack_k512_rolling()`](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_clean/ui_output_agent_53149_dispatch_22/source_2_gemm2_8x1.py#L1221)。
...
[最终10-buffer ABBA24](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_abba24_clean.json#L1)
使用固定的[67cefe2a pure快照](../../../../../tests/contrib/moe/results/multik_busy80/current_k512/ui_output_agent_42544_dispatch_22/source_2_gemm2_8x1.py#L1)
...
[首轮ABBA24](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_abba24.json#L1)
有明显长尾，未删除或筛选样本；它和
[首份91.253% ATT](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1/analysis.json#L1)
均保留作探索证据。首份ATT的initial查询出现10% busy，不作为clean终验；正式结论使用
下面的全新trace，不将两份trace拼接或取最高值。

### 七层结果：从pure到rolling

最终[机器账本](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_clean/analysis.json#L1)与
[采集命令及硬件状态](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_clean/hardware.json#L1)
...
[具体stage账本](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_clean/analysis.json#L58)
选择SE0/CU1/SIMD0/slot0的第一条active wave，N2/core3，MFMA ordinal608--639。
first/last PC index为1832/1908，均对应
[`fx.mma_atom_call()`](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_clean/ui_output_agent_53149_dispatch_22/source_2_gemm2_8x1.py#L1079)。
...
- 单N/drain：
  [N128/B1/padding0](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_contract_n128_b1_p0_r1.json#L1)、
  [N128/B33/padding128](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_contract_n128_b33_p128_r1.json#L1)。
- 跨N：
  [N256/padding32](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_contract_n256_b257_p32_r1.json#L1)、
  [N256/padding64](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_contract_n256_b257_p64_r1.json#L1)、
  [N2048/padding128](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_contract_n2048_b257_p128_r1.json#L1)，均B257/E8/TOPK4。
- 回退开关：
  [`MOE_8X1_ROLLING_EPILOGUE=0`](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_contract_n256_b257_p128_r0.json#L1)
  保留pure行为。正式[五K端到端回归](../../../../../tests/ops/moe/test_moe.py#L1359)为 **5 passed**。
...
- [候选源码快照](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_clean/ui_output_agent_53149_dispatch_22/source_2_gemm2_8x1.py#L1)
...
- [解码code表](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k512_v1_clean/ui_output_agent_53149_dispatch_22/code.json#L1)
...
[run_moe_8x1.py](../../../../../tests/flydsl/attn_4wave/tools/run_moe_8x1.py)，后处理只用[analyze_mfma_stall.py](analyze_mfma_stall.py)。
...
TOOLS="$REPO/experiments/attention/flydsl/attn_4wave/tools"
...
| --- | --- | --- | --- | --- |
| [已调优5×64单store](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_final_vs_single64_abba24.json#L1) | 0.986064 / 348.45 | **0.894944 / 383.93** | **0.908141 / [0.903181,0.913842]** | 24/24 |
| [原始5×64 pure](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_final_vs_pure_abba24.json#L1) | 1.964728 / 174.88 | **0.892584 / 384.95** | **0.453930 / [0.452500,0.454688]** | 24/24 |
...
[最终采集状态](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_final/hardware.json#L1)
记录before全机0%、managed全机0--1%；恢复查询busy为11%，不是全程负载监控数据，保留原值。
两轮ATT都报告，不选择最高一轮来隐瞒波动；硬件配置均恢复auto、PTL Disabled/N/A、NUMA on。

### 七层ATT与原因转移

证据为[最终账本](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_final/analysis.json#L1)及
[首轮账本](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_v1/analysis.json#L1)。
...
pure与v3证据分别为[原始账本](../../../../../tests/contrib/moe/results/multik_busy80/base_k320/analysis.json#L1)、
[协作预取账本](../../../../../tests/contrib/moe/results/multik_busy80/coop64_k320_v3/analysis.json#L1)。
...
[尾段实例](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_final/analysis.json#L66)：
SE0/CU1/SIMD0/slot0，N2/core2，ordinal384--399，PC index1438--1456。
对应[`mma()`历史调用](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_final/ui_output_agent_9579_dispatch_22/source_2_gemm2_8x1_k320.py#L294)，窗口
...
- 7组独立随机FP32参考检查：N128/B1/padding0、N128/B33/padding128、N256/B257/padding32/64、
  ...
  [生产宽度结果](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_final_contract_n2048_b257_p128_r1.json#L1)。
- 默认dispatcher的[七K端到端回归](../../../../../tests/ops/moe/test_moe.py#L1359)为 **7 passed**；
  分析器[10项CPU测试](../../../../../tests/flydsl/attn_4wave/tools/test_analyze_mfma_stall.py)通过，旧K512 JSON复算逐字节相同。
...
[独立builder快照](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_final/ui_output_agent_9579_dispatch_22/source_2_gemm2_8x1_k320.py#L1)
...
[code表](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k320_final/ui_output_agent_9579_dispatch_22/code.json#L1)
...
证据：[随机参考/输出契约](../../../../../tests/contrib/moe/results/multik_busy80/base_k640_check.json#L1)
relative-L2为0.003312115；[资源与七层账本](../../../../../tests/contrib/moe/results/multik_busy80/base_k640/analysis.json#L1)
有416条完整active wave、208个batch，16个采样SIMD均核验最大驻留2。
[采集状态](../../../../../tests/contrib/moe/results/multik_busy80/base_k640/hardware.json#L1)记录相同PTL/DPM协议。
...
[`DEDICATED_K640`](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1/ui_output_agent_56200_dispatch_22/source_2_gemm2_8x1.py#L85)
选择独立的[`pack_k640_rolling()`](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1/ui_output_agent_56200_dispatch_22/source_2_gemm2_8x1.py#L1329)，
...
[正式复测](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1_abba24_clean.json#L1)
使用10 rotating buffers、24轮ABBA/BAAB；每版本48个样本、24个paired ratio。
control固定为[7abaacbd pure快照](../../../../../tests/contrib/moe/results/multik_busy80/base_k640/ui_output_agent_49139_dispatch_22/source_2_gemm2_8x1.py#L1)。
...
[首轮ABBA24](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1_abba24.json#L1)
...
最终[机器账本](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1/analysis.json#L1)和
[采集状态/命令](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1/hardware.json#L1)
...
[具体stage账本](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1/analysis.json#L58)
为SE0/CU1/SIMD0/slot0，N2/core4，ordinal768--799，first/last PC index2117/2195，
对应[`fx.mma_atom_call()`](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1/ui_output_agent_56200_dispatch_22/source_2_gemm2_8x1.py#L1124)。
...
- [N128/B1/padding0](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1_contract_n128_b1_p0_r1.json#L1)、
  [N128/B33/padding128](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1_contract_n128_b33_p128_r1.json#L1)覆盖单N/drain。
- [N256/padding32](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1_contract_n256_b257_p32_r1.json#L1)、
  [N256/padding64](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1_contract_n256_b257_p64_r1.json#L1)、
  [N2048/padding128](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1_contract_n2048_b257_p128_r1.json#L1)，均B257/E8/TOPK4。
- [rolling禁用回退](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1_contract_n256_b257_p128_r0.json#L1)。

relative-L2范围0.003075--0.003365；[七K端到端回归](../../../../../tests/ops/moe/test_moe.py#L1359)为
...
- [源码快照](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1/ui_output_agent_56200_dispatch_22/source_2_gemm2_8x1.py#L1)
...
- [code表](../../../../../tests/contrib/moe/results/multik_busy80/rolling_k640_v1/ui_output_agent_56200_dispatch_22/code.json#L1)
...
- [随机torch-reference](../../../../../tests/contrib/moe/results/multik_busy80/current_k192_check.json#L1)：
  B257、E8、TOPK4、N2048，relative-L2为`0.0033148662`，finite；与原版control逐bit一致。
- [10-buffer ABBA24](../../../../../tests/contrib/moe/results/multik_busy80/current_k192_abba24.json#L1)：
...
- [七层机器账本](../../../../../tests/contrib/moe/results/multik_busy80/current_k192/analysis.json#L1)；
  raw trace位于同目录`ui_output_agent_27246_dispatch_22`，416条完整active wave、
  16个physical SIMD、208个resident batch，采样中没有early-exit wave。
- [实际采集命令及硬件状态](../../../../../tests/contrib/moe/results/multik_busy80/current_k192/hardware.json#L1)。
...
[完整测量](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k192_final_abba24.json#L1)
使用10 rotating buffers、24轮ABBA/BAAB，每版本48个样本、24个paired ratio，全量保留。
control为[67cefe2a pure快照](../../../../../tests/contrib/moe/results/multik_busy80/current_k192/ui_output_agent_27246_dispatch_22/source_2_gemm2_8x1.py#L1)。
...
[机器账本](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k192_v1/analysis.json#L1)及
[采集状态](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k192_v1/hardware.json#L1)
...
[N2/core1尾段](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k192_v1/analysis.json#L66)
位于SE0/CU1/SIMD0/slot0，ordinal224--239、PC index1169--1187，对应
[`mma()`历史调用](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k192_v1/ui_output_agent_41716_dispatch_22/source_2_gemm2_8x1_k192.py#L294)。
...
| --- | ---: | --- | --- |
| [首版四stage各一quarter](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k192_v1_abba4.json#L1) | 48KiB | 0.674543 ms / 305.63 | 保留，另做上面的ABBA24 |
| [独占scratch，读回跨compute延迟store](../../../../../tests/contrib/moe/results/multik_busy80/readpipe_k192_v2_abba4.json#L1) | 64KiB | 0.708324 ms / 291.05 | 撤回，未采ATT |
| [仅BK64 memory阶段各退休两个row-pair](../../../../../tests/contrib/moe/results/multik_busy80/tailretire_k192_v3_abba4.json#L1) | 48KiB | 0.708943 ms / 290.80 | 撤回，未采ATT |
...
- 7组[输出契约检查](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k192_final_contract_n2048_b257_p128_r1.json#L1)：
...
- 该轮默认七K加显式混合K192端到端用例为 **8 passed**；当时用缓存隔离确认真正传入128。
  清理后默认K192已覆盖混合实现，重复用例删除，改用[分流与旧参数拒绝测试](../../../../../tests/ops/moe/test_moe.py#L1380-L1420)；本轮结果见下一节。
  [分析器CPU测试](../../../../../tests/flydsl/attn_4wave/tools/test_analyze_mfma_stall.py)为 **14 passed**，包含四stage的前缀边界。
...
[独立builder快照](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k192_v1/ui_output_agent_41716_dispatch_22/source_2_gemm2_8x1_k192.py#L1)
...
[code表](../../../../../tests/contrib/moe/results/multik_busy80/mixed_k192_v1/ui_output_agent_41716_dispatch_22/code.json#L1)
...
[验证汇总](../../../../../tests/contrib/moe/results/multik_busy80/bk64_cleanup_validation.json#L1)保存精确源码身份、
逐指令哈希、资源、测试和硬件状态。随后按要求清理K128死代码与恒真条件、补per-tensor并重测矩阵，
新一轮结果见[当前汇总](../../../../../tests/contrib/moe/results/8x1_final_matrix/summary.json)。前面各节ATT仍归属于历史测量。
...
- [运行器](../../../../../tests/flydsl/attn_4wave/tools/run_moe_8x1.py#L197-L217)默认也统一为128；CLI的64选项继续服务于历史`--source/--control`快照，
...
共 **19 passed，28.34s**；分析器CPU回归 **14 passed，0.20s**。当前[测试集](../../../../../tests/ops/moe/test_moe.py)
...
| --- | --- | ---: | --- |
| K192，128+64 | [关](../../../../../tests/contrib/moe/results/multik_busy80/bk64_cleanup_default_k192_r0_contract.json#L1) / [开](../../../../../tests/contrib/moe/results/multik_busy80/bk64_cleanup_default_k192_r1_contract.json#L1) | 0.00330369 | 均逐bit相同 |
| K320，128+128+64 | [关](../../../../../tests/contrib/moe/results/multik_busy80/bk64_cleanup_default_k320_r0_contract.json#L1) / [开](../../../../../tests/contrib/moe/results/multik_busy80/bk64_cleanup_default_k320_r1_contract.json#L1) | 0.00329561 | 均逐bit相同 |
...
- [随机输出契约](../../../../../tests/flydsl/attn_4wave/tools/test_moe_8x1_quant.py)56项通过；七K端到端/分流42项，API/统计/分析器41项，合计139项。
  scalar版本七K及Hy3真实生产shape均finite、0 scratch；详细资源与源码哈希见
  [当前汇总](../../../../../tests/contrib/moe/results/8x1_final_matrix/summary.json)。
- 完整性能矩阵重测42点ABBA12并以14点ABBA48覆盖边界。最终8x1进入Qwen35 K512/K256的
  16K/32K及H3 8K--32K；Hy3、Xiaomi及8K端到端持平点按保守规则保留原路径。
  完整wall-time、有效TFLOPS、IQR与原始样本只维护在[最终性能报告](../../../../../tests/contrib/moe/MAIN_MERGE_PERFORMANCE_REPORT.md)。
...
同目录的[run_moe_8x1.py](../../../../../tests/flydsl/attn_4wave/tools/run_moe_8x1.py)负责down-only launch、随机reference、ABBA与
...
- [test_analyze_mfma_stall.py](../../../../../tests/flydsl/attn_4wave/tools/test_analyze_mfma_stall.py)：均匀/非均匀geometry的CPU回归测试。
- [run_moe_8x1.py](../../../../../tests/flydsl/attn_4wave/tools/run_moe_8x1.py)：统一down-only reference/ABBA/ATT工作负载及硬件门禁与恢复。
