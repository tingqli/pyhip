...
| `moe-evidence` | 核验下面引用的真实M128 sort256/sort128放置、均值及A消费域 | 默认读仓库中已有[真实MoE结果](../../../../../tests/flydsl/moe_8w_down/try/sort_alignment_20260913/default/results.json)，也可`--data`指定；Torch仅CPU加载原始tensor，不导入旧实验脚本 |
...
所以其他SE并没有等该长CTA结束。三次均找到147个这种post-prefix同CU复用证据。[原始样例与计时](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_20260913_v2/lds98304_one_straggler_0.json)、[时间线](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_20260913_v2/lds98304_one_straggler_0.svg)。
...
[详细记录](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_20260913_v2/lds98304_one_se_partial_release_0.json)、[时间线](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_20260913_v2/lds98304_one_se_partial_release_0.svg)。
...
[单文件GPU日志](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_gpu_v3.log)、[完整新结果](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_v3/results.json)、[SE1提前释放样例](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_v3/lds98304_one_se_partial_release_0.json)、[SE1时间线](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_v3/lds98304_one_se_partial_release_0.svg)。同一kernel内仍只有CTA局部barrier；这个更换目标SE的运行支持现象不是SE0特有。
...
旧受控MoE实验的[结果与方法](../../../../../tests/flydsl/moe_8w_down/try/xcd_schedule_20260913/README.md#4-两个因果对照不靠缓存也能复现收益)保留，本文单文件的 `moe-evidence` 无需导入那些旧脚本即可复查本次引用的真实M128时延与原始放置。
...
旧实际MoE中，每个B `(expert,OC)` 在identity和width4都只由一个XCD消费，不能说identity必然把同一B片段复制8次。B-only对照中identity／width8约111.836／113.325μs，几乎无swizzle收益，也是不能删掉的反例。[A/B局部性及PMC](../../../../../tests/flydsl/moe_8w_down/try/xcd_schedule_20260913/README.md#5-第二个机制a跨xcd复制而非只有b)。

### 5.6 修正sort128后，不能再沿用旧的padding解释

同进程正反两轮的[修正复测](../../../../../tests/flydsl/moe_8w_down/try/sort_alignment_20260913/default/results.json)：
...
单文件的 `moe-evidence` 还从真实放置复算出：原生sort128时，identity的1157个M块都由8个XCD消费；width4中1154个M块由2个XCD消费，**边界的3个M块由4个XCD消费**。这说明A域局部性仍有变化，但不能无条件断言原生布局所有M块都是2份，更不能在没有新PMC的情况下将剩余速度差全部量化为A缓存收益。[单文件复算日志](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_moe.log)。
...
[可放大SVG](se_dispatch_m128_mapping_comparison.svg)、[逐格数据及来源SHA256](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_m128_mapping_comparison.json)、[生成日志](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_plot_20260913.log)。使用同一次M128/sort128、Batch16384测试的实际GEMM放置回读与普通计时，**没有重新运行GPU**。
...
504.997是identity，不是width8；“使用8个物理XCD”也不等于“width8逻辑转置”。width4相对identity速度提升5.23%，相对width8提升1.74%，是两个不同的比较对象。新增绘图/指纹回归后的[18项内置测试](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_plot_20260913_tests.log)通过。

### 5.8 persistent与L2局部性可以兼得吗？

**可以设计成兼顾两者；persistent并不天然破坏L2局部性。** 需要区分“常驻执行方式”和“逻辑任务归属策略”。以下设计已有[独立M128/sort128实现](../../../../../tests/flydsl/moe_8w_down/try/xcd_persistent_20260913/README.md)，默认路径不变；实现通过正确性测试，不等于已经证明性能更快或L2命中率更高。

#### 为什么一个全局原子队列容易损失局部性

当前[主M256 persistent内核](../../../../moe/flydsl/moe_8w_down/moe_multistage_down.py)由所有常驻CTA从同一个计数器逐项领取任务。谁先完成谁先拿到下一个 `(M,OC)`，任务落在哪个XCD由完成顺序决定；对领取到的task再套swizzle，也不能恢复原来“任务固定归属某个XCD”的关系。
...
现有[M128实验工厂](../../../../../tests/flydsl/moe_8w_down/try/moe_multistage_down_m128.py)的 `static_ctas` 分支已经是这类 `linear_task += static_ctas` 后重新转置的枚举方式；它不是动态work stealing，不能平衡任务时长差异，而且当前selected active-B/spread配置并不直接允许开启该分支。不能把开关改成非零就宣称现有最佳流水得到保留或必然更快。
...
随后按请求增加了[独立入口](../../../../../tests/flydsl/moe_8w_down/try/xcd_persistent_20260913/moe_xcd_persistent.py)：8个128B间隔的XCD分片头、CTA内bundle状态、本地优先＋有限遍历其他分片、每次launch内部计数器清零。保留选定M128的PF3/ring4、activeB3/spread；不改默认width4或主M256。原调用者的一word counter保持不变，计数器由可复用workspace管理。
...
第一组正常计时（默认shape、同址正反两轮）也说明“理论上可以兼顾”不等于实际自动加速：原width4 Down **478.587μs**；256-worker/bundle4 **570.742μs**，256-worker/bundle8 **644.479μs**，512-worker/bundle4 **546.051μs**，均更慢。队列清零已计入，数值通过，不将该实现提升为默认。没有新ATT/PMC，因此这些数字尚不能区分额外queue/barrier、寄存器存活或访问时序各自的影响。[实现测量记录](../../../../../tests/flydsl/moe_8w_down/try/xcd_persistent_20260913/README.md#默认形状普通计时)。
...
[完整结果](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_20260913_v2/results.json)、[全部采样日志](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_20260913_v2.log)、[30份原始记录CPU复查](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_20260913_v2/recheck.log)、[10项CPU测试](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_20260913_v2/cpu_tests.log)。结果保留源码和编译二进制SHA256；每个sample有未裁剪的逐wave压缩JSON及单独摘要，第一轮关键case有SVG。

首次构建因ROCm7.2的 `hipFuncGetAttributes` 要求显式kernel pointer转换而失败；[失败日志](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_20260913.log)保留。修复该类型转换后重建到新目录，没有修改任务模式或筛除不符合预期的GPU样本。

本次单文件整理的检查包括：[15项内置测试](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_tests_v2.log)、[CPU模型输出](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_model.log)、[旧30样本重新核验](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_legacy.log)、[M128原始放置和时延复算](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_moe.log)，以及上面的新10次GPU采样。旧数据不重标为新运行，也不因旧脚本删除而改写其历史源码字段。

删除旧三个源文件后再次验证：[15项测试](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_tests_final.log)、[旧30样本](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_legacy_final.log)、[新10样本](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_new_final.log)、[教学模型](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_model_final.log)、[真实M128数据](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_moe_final.log)全部通过。所有入口均只由当前单文件提供，不依赖已经删除的脚本。

合并时保留了两个编译试错：[hipcc标准输入丢失目标架构](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_gpu.log)、[标准输入被device pass消耗后缺失host包装符号](../../../../../tests/flydsl/attn_4wave/tools/se_dispatch_singlefile_20260913_gpu_v2.log)。最终使用自动清理的临时源码进行正常host/device双编译；没有修改GPU任务逻辑或隐藏失败数据。