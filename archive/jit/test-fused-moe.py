...
def get_kernel(
    ...
    @pyhip.jit()
    def moe_gemm_kernel(J, p_input:"void*", p_weight:"void*", p_output:"void*", M:"int"):
        # use 16x16
        sizeof_bf16 = 2
        sizeof_f32 = 4
        ...
        if use_split_K:
            ...
            # convert to bf16?
            vaddr = J.gpr(lane_mod_16 * (N*sizeof_f32) + lane_div_16*(4*sizeof_f32))
            with J.ExecMask(lane_mod_16 < M[0]):
                J.global_store_dwordx4(vaddr, Creg[0], p_output)
                J.global_store_dwordx4(vaddr + 16*sizeof_f32, Creg[1], p_output)
