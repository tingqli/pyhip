...
HK = 2
S = 256
KV_LEN =  8000  # 我们的实际例子是8k 16K为主  262144是长上下文情况 保留
# KV_LEN = 512
DT = torch.bfloat16
BLOCK_SIZE = 1
...
if USE_REDUCE_JIT:
    ...
    def pa_reduce_jit(
        ...
        J.s_waitcnt(
            mod=f"lgkmcnt(0)"
        )  # 这类的wait应该可以自动生成，在第一次使用load指令结果的地方？
        ...
        with J.While(si < part_num) as loop:
            ...
            assert S % 64 == 0
            assert (
                not USE_REDUCE_JIT or S == 128
            ), "pa_reduce_jit 仍按 S=128 写死，S=256 请用 USE_REDUCE_JIT=False"
        ...
        if 0:
            J.Jump(
                "skip", (b[0] == 0) & (hq[0] == 0) & (s_warp_id[0] == 0), reverse=True
            )
...
workspace_buffer = torch.empty(
    ...
    * 4  # * nbyes_per_qo_elem
    + 2
    * (512 * HQ * 256)
    * 4,  # + 2 * (max_bs * self.num_head * self.max_num_partitions) * 4,
    dtype=torch.uint8,
)
# [B, H, S]
if FAKE_Q:
...
hip = pyhip.module(
    "pa-256.cpp",
    f"-D{HQ=} -D{HK=} -D{S=} -D{BLOCK_SIZE=} -DSCALE={scale} -D{KV_PART_SIZE=} -D{FAKE_Q=} -D{FAKE_K_IDX=} -D{OUTPUT_QK=}",
)
pa = hip.pa
...
qk_out = torch.ones(
    [B, HK, HQ // HK, div_up(KV_LEN, KV_PART_SIZE) * KV_PART_SIZE], dtype=torch.float32
)
pa(
    [B, HK, div_up(KV_LEN, KV_PART_SIZE)],
    [256],
    query.data_ptr(),
    key_caches[-1].data_ptr(),
    value_caches[-1].data_ptr(),
    kv_indptrs[-1].data_ptr(),
    kv_page_indices_[-1].data_ptr(),
    my_out_seg.data_ptr(),
    qk_out.data_ptr(),
    my_max.data_ptr(),
    my_sum.data_ptr(),
...
#    out, my_out, rtol=0.01, atol=0.01
#), "pa acc is wrong compare with aiter"
# print("pa acc ok compare with aiter")  # 开发时注掉这个

# check q*k
if 1:
    # ref 是 [B, HK, （HQ // HK）, S] @ [B, HK, S, KV_LEN] -> [B, HK, （HQ // HK）, KV_LEN]
    ref_qk = query.reshape(B, HK, HQ // HK, -1) @ key_caches[-1][
        1 : KV_LEN * B + 1
    ].reshape(B, -1, HK, S).permute(0, 2, 3, 1)
    ...
    if OUTPUT_QK:
        ...
        if len(idx[0]):
            print(f"idx = {idx}\nref_qk={ref_qk[idx]}\ncur={qk_out[idx]}")
        assert torch.allclose(
            ref_qk.to(torch.float32), qk_out, rtol=0.01, atol=0.01
        ), "pa qk is wrong"
        print("OUTPUT_QK ok")
    s = torch.softmax(ref_qk, dim=-1).to(value_cache.dtype)
    ref_out = s @ value_caches[-1][1 : KV_LEN * B + 1].reshape(B, -1, HK, S).permute(
        0, 2, 1, 3
    )
...
for _ in range(10):
    with pyhip.cudaPerf(
        B * HQ // HK * KV_LEN * S * 2 * 2, B * (HK * KV_LEN * S * 2 * 2), name="pa"
    ):
        pa(
            [B, HK, div_up(KV_LEN, KV_PART_SIZE)],
            [256],
