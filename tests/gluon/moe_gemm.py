import torch
from pyhip.contrib.gluon.moe_gemm import moe_2stage_gateup, moe_2stage_down
...
def _run_batch(kernel_type, B=1, weight_type=torch.bfloat16, TILE_M=16, TILE_N=32, run_count=10, HIDDEN_SIZE=2048, INTER_SIZE=1024, TOPK=8, E=128, TP=8):
    ...
    def run(hidden_states, w1, w2, topk_weight, topk_ids, w1_scale, w2_scale):
        ...
        if kernel_type == 'mxn_splitk_2s':
            ...
            if B * TOPK <= E:
                grid = B * TOPK
            x=moe_2stage_gateup(BLOCK_TILE_SIZE_M, BLOCK_TILE_SIZE_N)[(N1 // BLOCK_TILE_SIZE_N, grid)](
            ...
            # print(x.asm['amdgcn'])
            # assert 0
            x=moe_2stage_down(BLOCK_TILE_SIZE_M, BLOCK_TILE_SIZE_N)[(N2 // BLOCK_TILE_SIZE_N, grid)](
