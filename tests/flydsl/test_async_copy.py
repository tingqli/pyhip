...
def asycn_copy_padding(
    ...
    ac_tv_layout = fx.make_layout(
        ((8, 8, WAVE_NUM), 8), ((8 * WAVE_NUM * 8, 1, 8), WAVE_NUM * 8)
    )
    ...
    lds_layout_wr = fx.make_layout(
        ((8, 16), N_BLOCK), ((N_BLOCK, 8 * N_BLOCK + PADDING_ELEMS), 1)
    )
    ...
    lds_rd_tv_layout = fx.make_layout(
        ((16, 4, 2, 2), 8), ((1, 16 * WAVE_NUM // 2 * 8, 0, 16), WAVE_NUM * 16 // 2)
    )
    lds_rd_tiled = fx.make_tiled_copy(
        buffer_copy_atom, lds_rd_tv_layout, lds_rd_tile_mn
    )
...
def async_dma_copy_padding(
    ...
    asycn_copy_padding(arg_a_2d, B).launch(
        grid=(NUM_CU, 1, 1),
        block=(WAVE_NUM * 64, 1, 1),
        smem=(M_BLOCK * N_BLOCK + PADDING_NUM) * 2,
        stream=stream,
    )
...
def async_dma_copy_swizzle(
    ...
    arg_a_2d = fx.Tensor(fx.make_view(fx.get_iter(A), fx.make_layout((M, N), (N, 1))))
    asycn_copy_tile(arg_a_2d, B).launch(
        grid=(NUM_CU, 1, 1),
        block=(WAVE_NUM * 64, 1, 1),
        smem=M_BLOCK * N_BLOCK * 2,
        stream=stream,
    )
...
WG_F32_NELEMS = (
    2048 + 512 + 100
)  # 2660: part1=1轮DWORDx4, part2=1轮DWORD, part3=100 lane
...
def wg_buffer_load_lds_fp32(sA_ptr, gA, nelems, num_waves=WG_WAVES):
    ...
    if const_expr(n1 > 0):
        ...
        wave_off4 = fx.rocdl.readfirstlane(
            fx.Int32.ir_type, wave_id * (WARP * LOAD4)
        )  # wave_id*256
    ...
    if const_expr(rest1 > 0):
        ...
        wave_off1 = fx.rocdl.readfirstlane(
            fx.Int32.ir_type, wave_id * WARP
        )  # wave_id*64
        ...
        if const_expr(part3 > 0):
            ...
            with _mlir_ir.InsertionPoint(if_op.regions[0].blocks[0]):
                idx = load1 + load2 + tid
                dst = fx.make_view(
                    lds1, fx.make_layout(1, 1)
                )  # lds1 已前进到 load1+load2 基址
                src = fx.slice(gA, (None, fx.Int32(idx)))
                fx.copy(dma32, src, dst)
                _mlir_scf.YieldOp([])
...
if __name__ == "__main__":
    ...
    LDS_RESULT = (
        A.clone()
        .reshape(M // M_BLOCK, 8, M_BLOCK // 8, N)
        .permute(0, 2, 1, 3)
        .contiguous()
        .reshape(M, N)
    )
