...
def _fma_f32(lhs, rhs, acc, negate_acc=False):
    ...
    instruction = (
        "v_fma_f32 $0, $1, $2, -$3" if negate_acc else "v_fma_f32 $0, $1, $2, $3"
    )
    return fx.Float32(
        llvm.inline_asm(
            F32Type.get(),
...
def _fma_vec_f32(lhs, rhs, acc):
    return fx.Vector.from_elements(
        [_fma_f32(lhs[index], rhs, acc[index]) for index in range_constexpr(lhs.numel)],
        fx.Float32,
    )
...
def _scale_center_vec_f32(values, scale, center):
    return fx.Vector.from_elements(
        [
            _fma_f32(values[index], scale, center, negate_acc=True)
            for index in range_constexpr(values.numel)
        ],
        fx.Float32,
    )
...
def _cvt_f32_to_bf16(fragment):
    ...
    result.store(
        ((fragment.load().bitcast(fx.Uint32) + fx.Uint32(0x8000)) >> 16)
        .to(fx.Uint16)
        .bitcast(fx.BFloat16)
    )
    return result
...
def MHA(num_qo_heads, num_kv_heads, head_dim_qk, head_dim_v, page_size, is_causal):
    ...
    def attention_pipeline(
        ...
        mma_atom = fx.make_mma_atom(fx.rocdl.MFMA(32, 32, 16, fx.Float8E4M3FNUZ))
        k_permutation = fx.make_layout((8, 2, 2), (1, 16, 8))
        tiled_mma_qk = fx.make_tiled_mma(
            mma_atom,
            fx.make_layout((1, 4, 1), (1, 1, 0)),
            fx.make_tile(None, None, k_permutation),
        )
        tiled_mma_pv = fx.make_tiled_mma(
            mma_atom,
            fx.make_layout((1, 4, 1), (1, 1, 0)),
            fx.make_tile(None, None, k_permutation),
        )
        ...
        q_copy_atom = fx.make_copy_atom(fx.rocdl.BufferCopy128b(), fx.Float8E4M3FNUZ)
        ...
        k_fake = fx.Tensor(
            fx.make_view(
                fx.get_iter(k_tile),
                fx.make_layout((block_n, head_dim_qk), (head_dim_qk, 1)),
            )
        )
        v_fake = fx.Tensor(
            fx.make_view(
                fx.get_iter(v_tile),
                fx.make_layout((head_dim_v, block_n), (block_n, 1)),
            )
        )
        ...
        probability_operand = thread_mma_pv.make_fragment_B(
            fx.make_rmem_tensor(
        ...
        @fx.union
        class SharedStorage:
            k_lds: fx.Array[fx.Float8E4M3FNUZ, 2 * block_n * head_dim_qk, 16]
            o_lds: fx.Array[fx.BFloat16, block_m * (head_dim_v // 2), 16]

        shared = fx.SharedAllocator().allocate(SharedStorage)
        k_lds = shared.k_lds.peek().view(
            fx.make_layout(
                (block_n, head_dim_qk, 2),
                (head_dim_qk, 1, block_n * head_dim_qk),
            )
        )
        output_swizzle = fx.SwizzleType.get(3, 3, 3)
        o_lds_store = shared.o_lds.peek().view(
            fx.make_composed_layout(
                fx.static(output_swizzle),
                fx.make_ordered_layout(
                    (head_dim_v // 2, block_m),
                    (0, 1),
                ),
            )
        )
        o_lds_read = shared.o_lds.peek().view(
            fx.make_composed_layout(
                fx.static(output_swizzle),
                fx.make_ordered_layout(
                    (block_m, head_dim_v // 2),
                    (1, 0),
                ),
            )
        )
        ...
        prefetched_k = fx.make_rmem_tensor(
            fx.make_layout((2, 3), (1, 2)),
            fx.Uint32,
        )
        k_row = tid // 8
        ...
        def is_valid_static_block(block_index):
            return (
                const_expr(block_index >= 0)
                if const_expr(isinstance(block_index, int))
                else True
            )

        def global_load_k(block_index, page_id):
            if const_expr(is_valid_static_block(block_index)):
                prefetched_k.fill(0)
                for atom_index in range_constexpr(3):
                    chunk = k_chunk_in_group + atom_index * 8
                    d_group = chunk // 2
                    d_half = chunk & 1
                    source_offset = (
                        page_id * block_n * head_dim_qk
                        + d_group * block_n * 16
                        + k_source_row * 16
                        + d_half * 8
                    )
                    source = fx.make_view(
                        fx.get_iter(k_tile_u32) + source_offset // 4,
                        fx.make_layout(2, 1),
                    )
                    fx.copy(
                        k_global_copy_atom,
                        source,
                        prefetched_k[None, atom_index],
                    )
                return 3
            return 0

        def lds_store_k(block_index, stage):
            if const_expr(is_valid_static_block(block_index)):
                for atom_index in range_constexpr(3):
                    chunk = k_chunk_in_group + atom_index * 8
                    destination_offset = (
                        (stage & 1) * block_n * head_dim_qk
                        + k_row * head_dim_qk
                        + chunk * 8
                    )
                    destination = fx.make_view(
                        fx.get_iter(k_lds) + destination_offset,
                        fx.make_layout(8, 1),
                    )
                    fx.copy(
                        k_lds_store_atom,
        ...
        k_lds_copy_atom = fx.make_copy_atom(fx.UniversalCopy128b(), fx.Float8E4M3FNUZ)
        k_lds_copy = fx.make_tiled_copy_A(k_lds_copy_atom, tiled_mma_qk).get_slice(tid)

        v_copy_atom = fx.make_copy_atom(fx.rocdl.BufferCopy128b(), fx.Float8E4M3FNUZ)
        v_copy = fx.make_tiled_copy_A(v_copy_atom, tiled_mma_pv).get_slice(tid)
        num_v_loads = (
            fx.size(frag_v.shape).get_static_leaf_int * frag_v.dtype.width
        ) // 128
        num_k_fragment_bits = (
            fx.size(frag_k.shape).get_static_leaf_int * frag_k.dtype.width
        )

        frag_o.fill(0.0)

        def gemm_qk():
            for k_group in range_constexpr(head_dim_qk // 32):
                for k_atom in range_constexpr(2):
                    accumulator = frag_s[None, 0, 0]
                    fx.mma_atom_call(
                        mma_atom,
                        accumulator,
                        frag_k[None, 0, (k_atom, k_group)],
                        frag_q[None, 0, (k_atom, k_group)],
                        accumulator,
                    )
        ...
        def kv_step(
            ...
            if const_expr(is_valid_static_block(kv_block)):
                ...
                fx.copy(
                    v_copy_atom,
                    v_copy.partition_S(v_tile[None, None, page_id0]),
                    v_copy.retile(frag_v),
                )
            ...
            lds_store_k(kv_block + 1, lds_stage ^ 1)
            global_load_k(
                kv_block + 2,
                page_id2,
            )
            ...
            if const_expr(is_valid_static_block(kv_block)):
                ...
                for k_group in range_constexpr(2):
                    start = k_group * 8
                    probability_lo = fx.Vector.from_elements(
                        [
                            _pack_f32x4_to_fp8(
                                fx.Vector.from_elements(
                                    [
                                        probability[start + offset]
                    ...
                    probability_hi = fx.Vector.from_elements(
                        [
                            _pack_f32x4_to_fp8(
                                fx.Vector.from_elements(
                                    [
                                        probability[start + 4 + offset]
                                        for offset in range_constexpr(4)
                                    ],
                                    fx.Float32,
                                )
            ...
            if const_expr(is_valid_static_block(kv_block + 1)):
                fx.copy(
                    k_lds_copy_atom,
                    k_lds_copy.partition_S(k_lds[None, None, lds_stage ^ 1]),
                    k_lds_copy.retile(frag_k),
                )
        ...
        global_load_k(0, page0)
        lds_store_k(0, 0)
        global_load_k(1, page1)
        gpu.barrier()
        fx.copy(
            k_lds_copy_atom,
            k_lds_copy.partition_S(k_lds[None, None, 0]),
            k_lds_copy.retile(frag_k),
        )
        ...
        if const_expr(is_causal):
            ...
            pages_to_process = (intersecting_pages < num_kv_pages).select(
                intersecting_pages,
                num_kv_pages,
            )
        else:
        ...
        cshuffle_store = fx.make_tiled_copy_C(
            cshuffle_store_atom, tiled_mma_pv
        ).get_slice(tid)
    ...
    def process_work_item(
        ...
        query_end = fx.Int32(
            arith.minsi(
                arith.unwrap(query_start + block_m),
                arith.unwrap(cu_seqlens_q[batch_index + 1]),
            )
        )
        ...
        q_tile = fx.make_view(
            ...
            fx.make_ordered_layout(
                (block_m, num_qo_heads, head_dim_qk),
                (2, 1, 0),
            ),
        )
        q_tile = fx.rocdl.make_buffer_tensor(
            q_tile,
            max_size=False,
            num_records_bytes=query_len * num_qo_heads * head_dim_qk,
        )[None, qo_head, None]
        ...
        q_scale_tile = fx.rocdl.make_buffer_tensor(
            q_scale_tile,
            max_size=False,
            num_records_bytes=query_len * num_qo_heads * 4,
        )[None, qo_head]
        query_row = (tid // 64) * 32 + (tid & 31)
        qk_scale_log2 = (
            q_scale_tile[query_row] * k_scale * fx.Float32(qk_scale_log2_base)
        )

        o_tile = fx.make_view(
            fx.get_iter(output) + query_start * num_qo_heads * head_dim_v,
            fx.make_ordered_layout(
                (block_m, num_qo_heads, head_dim_v),
                (2, 1, 0),
            ),
        )
        o_tile = fx.rocdl.make_buffer_tensor(
            o_tile,
            max_size=False,
            num_records_bytes=query_len * num_qo_heads * head_dim_v * 2,
        )[None, qo_head, None]
    ...
    def attention_kernel_static(
        ...
        if const_expr(is_causal):
            ...
            balanced_work = ((physical_tile & 1) == 0).select(
                half_tile,
                works_per_head - 1 - half_tile,
            )
            affine_work = (
                physical_tile * causal_tile_step + causal_tile_offset
            ) % works_per_head
            current_work = (works_per_head == 256).select(
                affine_work,
                balanced_work,
            )
        else:
            ...
            while (batch_index < batch_size) & (current_work >= works_per_head):
                ...
                if head_index >= num_qo_heads:
                    ...
                    if batch_index < batch_size:
                        works_per_head = (
                            cu_seqlens_q[batch_index + 1]
                            - cu_seqlens_q[batch_index]
                            + block_m
                            - 1
                        ) // block_m
    ...
    def launch(
        ...
        q = fx.make_view(
            fx.get_iter(q),
            fx.make_ordered_layout(
                (num_query_tokens, num_qo_heads, head_dim_qk),
                (2, 1, 0),
            ),
        )
        k = fx.make_view(
        ...
        q_descale = fx.make_view(
            ...
            fx.make_ordered_layout(
                (num_query_tokens, num_qo_heads, 1),
                (2, 1, 0),
            ),
        )
    ...
    def callable(
        ...
        assert k.dtype == torch.float8_e4m3fnuz
        assert v.dtype == torch.float8_e4m3fnuz
        ...
        compiled = compiled_cache.get(static_schedule)
        if compiled is None:
            ...
            compiled_cache[static_schedule] = compiled
