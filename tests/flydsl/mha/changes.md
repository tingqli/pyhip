...
## 2026-09-08：gfx950同步最新分支并启动任意GPU空闲重测

- 从`4de089a`快进到`4adaf81eeb7d7fe334b4d256c47473a2422aa60f`。本地未提交的旧测试适配和文档修改先保存到
	stash `135a78c94d5828e96160eced9bf3f4698287774f`；未将已删除helper重新应用，原报告及原始结果保留。
- 当前MHA测试/生产源码未作新修改。GPU-free检查确认原生编译缓存开启、24个声明case中gfx950适用14个、48候选；
	计划每次性能运行2400个event，功能pytest预计6适用/8个FP8架构跳过。这些均为计划计数，尚非原生通过结果。
- 用户要求等待任意GPU空闲后重测，不再沿用上一轮非独占许可。已于03:22 UTC实际启动新入口的`--gpu auto`，
	严格执行连续3次、5秒间隔gfx/UMC均0%且无其他进程；之后自动依次运行功能pytest和性能pytest，各进程单独选卡。
- 初始8张设备均报告MI350X/gfx950/256 CU，利用率为0但仍有进程记录，因此尚未选卡。
	不因N/A名称或0字节忽略记录，不reset、不修改PTL/时钟/功耗、不终止其他任务。
- 当前状态和准确复现命令见[README.355.md](README.355.md)，[运行状态](results/gfx950-idle-20260908.kPxX0N/run-status.json)
	与[选卡采样](results/gfx950-idle-20260908.kPxX0N/all.idle.jsonl)已落盘。**本轮仍待空闲，未生成新性能结论。**
- 旧[2026-09-07非独占结果](results/mi350-10buf.b5VIea/validation.json)保持原值，不能替代当前单文件/新集合/空闲条件验收。

## 2026-09-08：按用户要求低于2%即可开始，完成gfx950重测

- 用户更新条件为“只要gpu占用率低于2%即可开始”。增加显式`--max-gpu-utilization 2`及pytest环境变量
	`PYHIP_MHA_MAX_GPU_UTILIZATION=2`；每卡连续3次、5秒间隔gfx/UMC均严格低于门槛即可选中，允许驻留进程。
	默认0保留MI308原来的零利用率且无其他进程规则，不将本轮许可隐式应用于其它机器。
- 只修改主测试文件中的选卡/守卫/报告逻辑：选卡前复查利用率、保留进程记录和原PTL/BDF/锁检查；
	每case前后记录负载，不以自己的计时负载判成其他任务争用。JSON/Markdown均标为低负载启动、非独占。
	门槛只约束选卡/开始，不保证测量期间持续低于2%；current模式不允许绕过正门槛的起始检查。
- 原严格等待在03:43被主动中断，等待1266.585秒、没有执行测试；只结束本任务自己的等待及监控，旧日志保留。
	新结果从独立目录重新开始，不拼接旧等待或旧性能样本。
- 本轮03:50:57–03:53:45 UTC完成CLI、功能pytest及性能pytest。三次均实际选择SMI GPU0、PCI `0000:05:00.0`，
	MI350X/gfx950/256 CU，ROCr映射为逻辑0并核验；选中时gfx/UMC均0%，各2条其他进程记录按许可保留。
- CLI和性能pytest**各自14个适用case、48候选、2400个event全部通过**，10个架构case不执行；
	功能pytest**6通过、8个gfx942 FP8跳过**，性能pytest**14通过、10跳过**。没有新增CPU测试或恢复已删除专项。
- 已独立核验两轮所有候选的10buffer精度/两次逐位重复、输入输出去别名、每轮0–9、50样本、gather20个K/V workspace与双dispatch、
	逻辑字节/FLOPs/TFLOPS/GB/s/中位数、源码身份及三次选卡采样。生产4个kernel及DSL与HEAD逐字节一致。
- 进程总耗时CLI46.429秒、功能24.682秒、性能pytest97.001秒；合计168.113秒，不含旧严格等待和文档整理。
	FlyDSL原生编译缓存保持开启、未清理或重定向；没有硬件setter/reset或终止其他GPU任务。
- [README.355.md](README.355.md)前置同一轮CLI完整48行及复现命令，旧非独占报告保留为历史；
	[本轮目录](results/gfx950-lowload2-20260908.yFE1Zz)、[验证汇总](results/gfx950-lowload2-20260908.yFE1Zz/validation.json)。
	实际为MI350X，不冒充MI355X原生或独占性能验收；本轮未commit/push。

## 2026-09-08：统一平台报告格式，删除旧950文档

- 按用户要求删除旧950平台文档，入口及MI308文档的gfx950引用改为[README.355.md](README.355.md)。
- 355报告按[README.308.md](README.308.md)统一为8节：环境、完整数据/耗时、10-buffer协议、参考路径、指标、CLI、case/pytest、缓存与跨机边界。
  保留本机同一轮48条数据和真实MI350X/低负载非独占说明；FP8明确为本机不适用，不拷贝MI308性能值。
- 删除报告正文的重复历史表及已失效命令，历史JSON/JUnit/日志不删除、不改写；第8节列明完成测试所需的主入口负载门槛、守卫及报告改动。
- 本次仅整理文档，不修改运行代码、kernel、参数集合或计时口径，不重复执行GPU测试。