...
"""
based on gemmv1, introduced gather/scatter
"""
...
def compile_v1(M, N, K, BLOCK_M, BLOCK_N, BLOCK_K):
    ...
    def gemm_kernel_v1(A: fx.Tensor, B: fx.Tensor, Aidx: fx.Tensor, C: fx.Tensor,):
        ...
        Aidx = fx.rocdl.make_buffer_tensor(Aidx, False)
        ...
        frag_rowsC = frag_C.load_scatter_rows(bAidx)

        for k, state in range(fx.Index(0), fx.Index(num_blocks_k), fx.Index(1), init=[]):
            frag_A.copy_from(A, rows = frag_rows, cols = col_tview[None, None, None, k])
        ...
        col_tensor = fx.make_view(fx.make_int_tuple(0), fx.make_layout((BLOCK_M, BLOCK_N, num_blocks_n), (0, 1, BLOCK_N)))
        col_tview = frag_C.partition_D(col_tensor)
        frag_C.copy_to(C, rows = frag_rowsC, cols = col_tview[None, None, None, bid_n])
    ...
    def launcher(A: fx.Tensor, B: fx.Tensor, Aidx: fx.Tensor, C: fx.Tensor,stream: fx.Stream):
        ...
        B = fx.flat_divide(B, (BLOCK_N, BLOCK_K))  # (BLOCK_N, BLOCK_K, num_blocks_n, num_blocks_k)
        print(Aidx, B)
        #C = fx.flat_divide(C, (BLOCK_M, BLOCK_N)) # (BLOCK_M, BLOCK_N, num_blocks_m,num_blocks_n)
        grid_m = fx.get_scalar(Aidx.shape[1])
        grid_n = fx.get_scalar(B.shape[2])
...
def test_gemm(compile, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K):
    ...
    Aidx = torch.randperm(M, dtype=torch.int32).cuda()
    ...
    pyhip.run_perftest(kernel, *args,
                       ...
                       num_warmup=1, num_iters=10,
                       num_name=f"gemm_{M}_{N}_{K}_{BLOCK_M}_{BLOCK_N}_{BLOCK_K}",)

    C0 = A[Aidx, :].to(out_dtype) @ B.to(out_dtype).T
    expected = torch.empty_like(C0)
    expected[Aidx, :] = C0
