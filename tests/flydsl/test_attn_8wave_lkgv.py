    ...
    def attn_kernel(Q_: fx.Tensor, K_: fx.Tensor, V_: fx.Tensor, O_: fx.Tensor, M: fx.Int32, N: fx.Int32):
        ...
        def online_softmax(frag, ml_states):
            ...
            for mt in range_constexpr(2):
                scores = _eltwise_op("v_mul_f32", frag[None, None, mt].load(), sm_scale_log2)
            ...
            frag_bf16 = fx.select(frag_bf16, [0,2,1]) # [fragv, n_bn, n_bm] -> [fragv, n_bm, n_bn]
