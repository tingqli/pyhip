...
def MHA(M, N, D, BM, BN, H):
    ...
    def attn_kernel(Q_: fx.Tensor, K_: fx.Tensor, V_: fx.Tensor, O_: fx.Tensor, S_: fx.Tensor):
        ...
        def ds_store_k(frag_id, lds_buff_id):
        ...
        def ds_store_v(frag_id, lds_buff_id):
        ...
        loop:======================
            ...
            # 此处发起LDS写入安全，因为2个barrier之前已经完成了:
            #   - LDS的访问
            #   - prefetch k1 & v0的数据就位
            #
            save k2 & v1 to LDS0  <========= on-the-way: k3v2  k2v1
            prefetch k4 & v3      <========= k4&v3 can reuse fragment of k2v1

            ds_read K1 from LDS1 <====== a huge read from LDS: should interleave with online-softmax
            ds_read V0 from LDS1 <====== a huge read from LDS: should interleave with online-softmax

            softmax(S)=>P

            wait ds_read finish， wait prefetch k3 & v2 finish
            ::::::::: wave-group barrier ::::::::: 切换调度
            ...
            ds_read V1 from LDS0

            softmax(S)=>P

            wait ds_read finish， wait prefetch k4 & v3 finish
            ::::::::: wave-group barrier ::::::::: 切换调度
        ...
        def kv_step(block_n, lds_buff_id, all_is_valid = False):
            def is_valid_block_n(bn):
                if fx.const_expr(all_is_valid):
                    return True
            ...
            prefetch_k = block_n + 2
            prefetch_v = block_n + 1
            if fx.const_expr(is_valid_block_n(prefetch_k)):
                ds_store_k(prefetch_frag_id, lds_buff_id)

            if fx.const_expr(is_valid_block_n(prefetch_v)):
                ds_store_v(prefetch_frag_id, lds_buff_id)

            vm_cnt = 0
            if fx.const_expr(is_valid_block_n(block_n + 4)):
                global_load_k(block_n + 4, prefetch_frag_id)
                vm_cnt += num_vm_cnt_load_k

            if fx.const_expr(is_valid_block_n(block_n + 3)):
                global_load_v(block_n + 3, prefetch_frag_id)
            ...
            if fx.const_expr(is_valid_block_n(block_n + 0)):
                flyobj.load_tiled_mma_fragA(tmma2, lds_v, [None, None, lds_buff_id^1], dst=fragV)

            # softmax(S)=>P
            rocdl.sched_barrier(0)
            s_waitcnt(vm_cnt, 0) # ::::::::: wave-group barrier ::::::::: 切换调度
            gpu.barrier()
        ...
        if fx.const_expr(0):
            for block_n in fx.range_constexpr(0, num_blocks_n, 2):
                kv_step(block_n, 0, True)
                kv_step(block_n + 1, 1, True)
            return

        for block_n, state in range(0, num_blocks_n, 2, init=[]):
            kv_step(block_n, 0, True)
            kv_step(block_n + 1, 1, True)
