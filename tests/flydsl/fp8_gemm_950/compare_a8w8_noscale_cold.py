...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Compare FlyDSL E4M3 A8W8 with and without scale against "
            "Gluon E5M2 A8W8"
        )
    )
...
def permute_scale(scale, rows):
    ...
    padding = torch.full(
        (padded_rows * 4,), 127, device="cuda", dtype=torch.uint8
    )
...
def main():
    ...
    if args.case in ("both", "noscale"):
        benchmark(
            "flydsl-e4m3", flydsl_launch, args.clones, args.runs, flops, rw_bytes
        )
    if args.case in ("both", "scaled"):
        benchmark(
    ...
    if args.gluon:
        ...
        benchmark(
            "gluon-e5m2", gluon_launch, args.clones, args.runs, flops, rw_bytes
        )
