...
# PYTHONPATH=/tmp/triton-gfx950-v11-site:/mywork/pyhip LLVM_PASS_PLUGIN_PATH=/mywork/gfx950-gluon-tutorials/plugins/llir_scheduler
# /libLlirSched.so LLVM_PASS_PLUGIN_KEEP_TARGET_MACHINE=1 TRITON_FORCE_MFMA_AGPR=1 TRITON_AMDGCNAS_PLUGIN=1 TRITON_CACHE_DIR=/tmp/a8w8_noscale_compare_cache python compare_a8w8_noscale_cold.py
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare no-scale FlyDSL E4M3 and Gluon E5M2 A8W8 GEMMs"
    )
...
def make_fp8_clones(shape, clones, generator, label):
    source = torch.randn(
        shape,
        device="cuda",
        dtype=torch.float32,
    ...
    e5m2_base = source.to(torch.float8_e5m2)
    del source

    e4m3 =  [e4m3_base.clone() for _ in range(clones )]
    e5m2 =  [e5m2_base.clone() for _ in range(clones )]
...
def make_shared_inputs(m, n, k, clones, seed):
    generator = torch.Generator(device="cuda")
    generator.manual_seed(seed)
    fly_a, gluon_a = make_fp8_clones((m, k), clones, generator, "A")
    fly_b, gluon_b = make_fp8_clones((n, k), clones, generator, "B")
    ...
    print(
        ...
        "gluon_dtype=float8_e5m2 scale=none",
        flush=True,
    )
    return fly_a, fly_b, fly_c, gluon_a, gluon_b, gluon_c


def make_flydsl_launcher(m, n, k, inputs_a, inputs_b, outputs):
...
def main():
    ...
    inputs = make_shared_inputs(args.m, args.n, args.k, args.clones, args.seed)
    fly_a, fly_b, fly_c, gluon_a, gluon_b, gluon_c = inputs
