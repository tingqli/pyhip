...
def test_mfma_32x32x16():
    @pyhip.jit("(void* A, void* B, void* C, int strideAB, int strideC)")
    def kernel(J):
        ...
        buff_a = J.Buffer(pA, 32*16*sizeof_half)
        buff_b = J.Buffer(pB, 32*16*sizeof_half)
        ...
        buff_a.load_dwordx4(matA, voffset, 0)
        buff_b.load_dwordx4(matB, voffset, 0)
        J.s_waitcnt(mod=f"vmcnt({0})")

        # v_mfma_f32_32x32x8_f16         vdst:f32x16, vsrc0:f16x4,  vsrc1:f16x4,  src2:f32x16  cbsz abid blgp
        J.v_mfma_f32_32x32x8_f16(matC0[0:15], matA[0:1], matB[0:1], 0)
    ...
    C = torch.randn(32, 32, dtype=torch.float)
    ref = torch.nn.functional.linear(A, B).to(dtype=torch.float)
    kernel([1],[64], A.data_ptr(), B.data_ptr(), C.data_ptr(), 16, 32)
