...
JIT的编程方式，即使有表达式等语法糖的加持，也是要比单纯HIP编程要更为复杂一些。具体需要付出多大代价，能够带来什么帮助还是需要实践来检验。但是其使用的场景比较明确的：

 - 支持纯汇编级别的代码生成的灵活性，这是其立足的根本。由于jit带来的可定制化代码生成，以及一些语法糖加持，可以做到简化汇编书写，提高生产效率，灵活定制不同配置；
   -  实现可配置gemm(手工指令调度)

 - 支持寄存器自动分配之后，jit代码可以实现更多的类似伪指令一样的功能，类似STL，Clib一样的？

一些实验性的思路可以基于此进行尝试，但是并非其立足根本：

 - 潜在可以支持指令自动排序功能？这要求对指令操作数数据依赖关系建模，目前这个信息是没有显式实现的，并没有对操作数连接关系显式建模；
 - 能否支持类似CM中matrix那样的高级抽象，或者更进一步，类似cutlass/CK那样的block级别的Tile抽象？对这样的抽象可以支持layout，直接提供数据存取的函数？甚至更进一步，提供eltwise的计算函数，不论如何，这样的函数在向一个基本块中产生代码之后，指令排序是否可以自动完成？
