...
可以视为一种汇编助记符的扩展，例如加减乘，移位，与/或等指令,但是要想生成正确的指令，还需要知道寄存器类型，数据类型等信息。这些信息在高级语言中存在，但是在JIT中却没有，除非我们直接引入寄存器变量概念，变量就具有类型，就能生成正确的指令。如果对无类型的寄存器施加表达式，jit可以直接报错，也就是说类型信息是可选的。只有使用表达式时才会参考类型信息。
