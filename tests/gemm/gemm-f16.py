...
K = 8192
StrideK = 128*1024
# buffer_load_dwordx4 performance suddenly drops (by-half) when stride > 256KB
# 
StrideK = K*8
...
if len(sys.argv) == 1:
    gemm_tile_256x256x32([1],[256], A.data_ptr(), B.data_ptr(), StrideK, out.data_ptr(), N, StrideK//32)
...
for i in range(2):
    with pyhip.cudaPerf(M*N*K*2*num_CU, (M*K*2+K*N*2)*num_CU):
        gemm_tile_256x256x32([num_CU],[256], A.data_ptr(), B.data_ptr(), StrideK, out.data_ptr(), N, K//32)
