...
```bash
python tests/ops/gr_read/test_gr_read.py --gpu 3
...
数据来源：[完整结果与各阶段样本](../../contrib/gr_read/results/readme_refresh_20260920/run/summary.json)、[原始测试日志](../../contrib/gr_read/results/readme_refresh_20260920/benchmark.log)、[样本／门禁／地址复核](../../contrib/gr_read/results/readme_refresh_20260920/summarize.json)。这是当前版本的一轮完整测量，不与不同场次的历史最佳值拼接，也不是优化前后的同址配对比较。
