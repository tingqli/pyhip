...
def moe_gemm_mxfp4(J, wg_M, wg_N,
    ...
    #with J.If(e_idx[0] != 1): J.s_endpgm()
    ...
    if gate_up:
        ...
        for m in range(nrM):
            #J.s_waitcnt(mod=f"lgkmcnt({nrM-1-m})")
            vrows[m] = vrows[m] & 0xFFFFFF
            vaddrs[m] = vrows[m] * stride_c + (J.lane_id//16) * (J.sizeof_DW * 2) + warp_n*(4*J.sizeof_DW2)
        ...
        for m in range(nrM):
            with J.ExecMask(vrows[m] < num_tokens[0]):
                for n in range(nrN):
                    J.v_pk_mul_f32(vf32[0:1], mfma_C[m,n,0:1], vweights[m])
                    J.v_pk_mul_f32(vf32[2:3], mfma_C[m,n,2:3], vweights[m])
                    J.uni_cvt_pk_bf16_f32(vbf16[0], vf32[0], vf32[1]) 
                    J.uni_cvt_pk_bf16_f32(vbf16[1], vf32[2], vf32[3])
                    J.global_atomic_pk_add_bf16(vaddrs[m], vbf16[0], output, mod=f"offset:{n*4*J.sizeof_DW2}")
                    J.global_atomic_pk_add_bf16(vaddrs[m], vbf16[1], output, mod=f"offset:{n*4*J.sizeof_DW2 + J.sizeof_DW}")
